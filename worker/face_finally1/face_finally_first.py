...
def facesdetecter(image):
    ...
    for (x, y, w, h) in faces:
        ...
        frame = cv2.circle(image, center=(x + w // 2, y + h // 2), radius=w // 2, color=(0, 255, 255),
                           thickness=2)  # 圆圈标识
        #为了节省运算，可考虑先锁定脸部，在脸部的基础上进行其他的检测
        facearea=image
        for (x, y, w, h) in eyes:

            frame = cv2.rectangle(facearea, (x, y), (x + w, y + h), (255, 0, 0), 2)  # 画框标识眼部
    ...
    if len(eyes) > 1:
        ...
        for (x, y, w, h) in rect_eyes:
            ...
            _, contours, hierarchy = cv2.findContours(thresh_eyes, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)  # 寻找前景
        ...
        for (x, y, w, h) in rect_mask:
            ...
            _, contours, hierarchy = cv2.findContours(thresh_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)  # 寻找前景
    ...
    fps = 1 / (end - start)  # 帧率
    frame = cv2.putText(image, "fps:{:.3f}".format(fps), (550, 15), cv2.FONT_HERSHEY_COMPLEX, 0.5, (255, 255, 0), 1)  # 绘制
